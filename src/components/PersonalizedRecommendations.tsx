        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {recommendations.map((rec) => (
            <Link key={rec.id} href={getRecommendationUrl(rec)}>
              <Card className="group hover:shadow-lg transition-all duration-300 border border-[var(--border-subtle)] hover:border-orange-200">
                <CardContent className="p-4">
                  <div className="space-y-3">
                    ...
                    {/* Excerpt */}
                    {rec.excerpt && (
                      <p className="text-xs text-[var(--text-strong)] line-clamp-2">
                        {rec.excerpt}
                      </p>
                    )}
