...
async function getPerformanceData(scope: string = 'ALL') {
  try {
    // Import the performance API logic directly instead of using fetch
    ...
    type PerformanceScope = 'ALL' | 'YTD' | '1Y' | '90D' | string
    type CachedPerformanceData = {
      stats: any
      equitySeries: any[]
      drawdownSeries: any[]
      monthlyReturns: any[]
    ...
    const closedTrades = filteredTrades.filter((t: any) => t.status === 'closed').length
    const openTrades = totalTrades - closedTrades
    
    // Calculate R-multiples from closed trades
    const rMultiples = filteredTrades
      .filter((t: any) => t.status === 'closed' && t.rMultiple !== null && t.rMultiple !== undefined)
      .map((t: any) => t.rMultiple as number)
    ...
    const performanceData: CachedPerformanceData = {
      ...
      tradeStats: {
        ...
        closedTrades,
        openTrades,
        avgHoldDays: stats.avgHoldDays || 0
      },
      rMultiples
    }
...
async function getTrades(scope: string = 'ALL'): Promise<any[]> {
  try {
    const { prisma } = await import('@/lib/prisma')
    const { filterTradesByScope } = await import('@/lib/perf')
            ...
            <div className="flex items-center gap-1">
              {['ALL', 'YTD', '1Y', '90D'].map((timeRange) => (
                <Button
                  ...
                  asChild
                >
                  <a href={`/portfolio/performance?scope=${timeRange}`}>
                    {timeRange}
                  </a>
                </Button>
              ))}
