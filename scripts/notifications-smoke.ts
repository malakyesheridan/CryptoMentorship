...
async function setPrefs(userId: string, overrides: ToggleOverrides = {}) {
  await prisma.notificationPreference.upsert({
    ...
    update: {
      ...
      inAppEnabled: true,
      inApp: true,
      email: overrides.emailEnabled ?? true,
      onSignal: overrides.portfolioUpdatesEmail ?? true,
      onEpisode: overrides.cryptoCompassEmail ?? true,
      onResearch: overrides.learningHubEmail ?? true,
      onMention: overrides.communityMentionsEmail ?? true,
      onReply: overrides.communityRepliesEmail ?? true,
    },
    create: {
      userId,
      emailEnabled: overrides.emailEnabled ?? true,
